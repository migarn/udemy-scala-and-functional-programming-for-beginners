...
object TuplesAndMaps extends App {
  ...
  val phonebook2 = Map(("Jim", 555), "Daniel" -> 789, ("JIM", 548)).withDefaultValue(-1)
  println(phonebook2.map(pair => pair._1.toLowerCase -> pair._2)) // careful with mapping keys
  ...
  def nPeopleWithNoFriends(network: Map[String, Set[String]]): Int =
    // network.filterKeys(person => network(person).isEmpty).size
